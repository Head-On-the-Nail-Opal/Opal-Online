...
public class MainMenuScript : MonoBehaviour {
    ...
    {
        if(mainDisplay.getCurrentOpal() == null)
        {
            return;
        }
        mainDisplay.getCurrentOpal().setOpal(null);
        if(mainDisplay.getCurrentOpal() != null && ((!checkRepeats(blueTeam, mainDisplay.getCurrentOpal()) && !checkRepeats(redTeam, mainDisplay.getCurrentOpal())) && !checkRepeats(greenTeam, mainDisplay.getCurrentOpal()) && !checkRepeats(orangeTeam, mainDisplay.getCurrentOpal()) || (dupes == 1 && !checkRepeats(getTeam(currentTeam), mainDisplay.getCurrentOpal()))) && !full)
        {
            if (dupes == 0)
            {
                foreach (PlateScript p in displayOpals)
                {
                    if (p.getOpal().getMyName() == mainDisplay.getCurrentOpal().getMyName())
                    {
            ...
            if (currentTeam == "blue")
            {
                displayPlates[0].addOpal(mainDisplay.getCurrentOpal());
                blueTeam.Add(mainDisplay.getCurrentOpal());
                updateCurrentTeam();
            }
            else if (currentTeam == "red")
            {
                displayPlates[1].addOpal(mainDisplay.getCurrentOpal());
                redTeam.Add(mainDisplay.getCurrentOpal());
                updateCurrentTeam();
            }
            else if (currentTeam == "green")
            {
                displayPlates[2].addOpal(mainDisplay.getCurrentOpal());
                greenTeam.Add(mainDisplay.getCurrentOpal());
                updateCurrentTeam();
            }
            else if (currentTeam == "orange")
            {
                displayPlates[3].addOpal(mainDisplay.getCurrentOpal());
                orangeTeam.Add(mainDisplay.getCurrentOpal());
                updateCurrentTeam();
            }
        }
        ...
        {
            full = true;
            startButton.transform.position = new Vector3(0, 16f, -3);
            menuState = "Play";
        }
    }
    ...
    public void setTeamDisplays()
    {
        ...
        teamTemp.transform.localPosition = new Vector3(-8.4f,19f, -0.6f);
